...
import { isBehavior, assertionOf } from '../../knowledge/sequence/structure.mjs';
import { drawingOrder } from './structure.mjs';
import { blockLines, regionLines } from './frames.mjs';
import { assertedEntry, entryStatus } from '../../knowledge/sequence/entry.mjs';
import { placementCaption } from './entry.mjs';
...
export function sequenceEntryHTML(model, evidenceURL) {
  const entry = assertedEntry(model);
  const participant = entry && model.participants.find((p) => p.id === entry.value.participantRef);
  const step = entry && model.steps.find((s) => s.id === entry.value.itemRef);
  const block = entry && model.blocks?.find((b) => b.id === entry.value.itemRef);
  const title = participant ? `Starts with ${participant.label}` : `Starting point ${entryStatus(model)}`;
  const description = entry ? `First ${block ? 'block' : 'step'}: ${(step ?? block).label}` : model.entry?.point.reason ?? 'No starting point is declared in this source.';
  return `<section class="sequence-entry-summary" aria-label="Sequence start"><div class="sequence-entry-copy"><span class="detail-key">Workflow entry: ${entryStatus(model)}</span><h2>${escape(title)}</h2><p>${escape(description)}</p><p class="purpose">Trigger: ${model.entry?.trigger.status ?? 'not declared'}</p></div><div class="sequence-entry-actions">${entry ? '<button id="sequence-start">Go to start</button>' : ''}<a class="header-link" href="${escape(evidenceURL)}">Inspect entry & trigger ↗</a></div></section>`;
}
export function sequenceSVGMarkup(layout, skin = 'standard', embed = true) {
  const model = layout.model, behavior = isBehavior(model), entry = assertedEntry(model);
  const attrs = (ref, label) => `class="record" data-ref="${escape(ref)}"${entry?.value.participantRef === ref ? ' data-sequence-entry="participant"' : entry?.value.itemRef === ref ? ' data-sequence-entry="item"' : ''} tabindex="0" role="button" aria-label="Inspect ${escape(label)}"`;
...
${embed ? `<metadata id="waxwing-source" data-encoding="base64">${Buffer.from(canonical(layout)).toString('base64')}</metadata>` : ''}
