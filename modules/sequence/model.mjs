...
export function validateSequenceModel(model) {
  if (!shape(model)) return { ok: false, diagnostics: shape.errors.map((error) => ({ code: 'sequence/schema', path: error.instancePath, message: error.message })) };
  ...
  for (const collection of ['participants', 'steps', 'notes', 'sources', 'documents']) for (const [i, record] of model[collection].entries()) {
  ...
  for (const order of candidates(model.order)) {
  ...
  for (const [i, note] of model.notes.entries()) for (const ref of note.subjectRefs) reference(ref, ['participants', 'steps'], `/notes/${i}/subjectRefs`);
  diagnostics.push(...documentDiagnostics(documentModel(model)));
  return { ok: !diagnostics.length, diagnostics, summary: { participants: model.participants.length, steps: model.steps.length, documents: model.documents.length,
    ...
    limits: 'Checks a partial current scenario, references, and asserted order. Does not verify evidence or establish timing, concurrency, or completeness.' };
}
