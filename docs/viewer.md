...
```js
import { inspectReadability } from 'waxwing/layout';
...
```js
import { selectHighlights } from 'waxwing/render';
const { refs, findings } = selectHighlights(json1, graph, { mode: 'unknown' });
```

`graph` is a graph selection from `waxwing/graphs` or JSON 1's `graphs[]`.
